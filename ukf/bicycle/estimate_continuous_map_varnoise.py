...
def filtering(sim, tracker, Rs=None):
    if Rs is None:
    ...
    __get_initial_readings(sim, tracker)
...
def run_tracker():
    ...
    # actual filtering
    Rs = []
    for var in np.linspace(5e-5, 5e-3, num_samples):
        Rs.append(R_proto * var)
    readings, filtered, Ps, R_estimations = filtering(sim, tracker, Rs)

    # plot_noise_matrices_with_data(R_estimations[20:], R_proto * sim_var,
    #                               filtered, readings)
    Rs = np.asarray(Rs)
    plot_noise_matrices(R_estimations, Rs)
