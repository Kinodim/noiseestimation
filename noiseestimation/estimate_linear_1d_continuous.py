...
def run_tracker():
    ...
    measurement_std_list = np.linspace(measurement_std / 5, measurement_std * 1, sample_size / 2)
    measurement_std_list = np.concatenate( (measurement_std_list, list(reversed(measurement_std_list)) ))
    ...
    for idx, _ in enumerate(measurement_std_list):
        ...
        if(idx < estimation_sample_size or idx % (estimation_sample_size / 10) != 0):
            print idx
            continue
