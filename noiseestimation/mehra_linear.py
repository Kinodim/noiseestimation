...
dt = 0.1
measurement_std = 2.
measurement_std_list = np.asarray([measurement_std] * 2000)
...
tracker.F = np.array([[1, dt],
                      [0,  1]])
q = 0#Q_discrete_white_noise(dim=2, dt=dt, var=0.001)
tracker.Q = q
tracker.H = np.array([[1, 0]])
tracker.R = measurement_std**2
...
cor = Correlator(residuals[100:], 100)
axarr[1].plot(cor.autocorrelation()[1:],'ro')
print cor.isWhite()
